    ...
    async def on_message(self, message):
        if message.author != self.user:
            ...
            if type(message.channel) != discord.channel.DMChannel:
                ...
                self.compute_massage(Message((from_id, "DS"), text, author_id, chat_name=chat_name, is_owner=is_owner, photos=photo))
            else:
                self.compute_massage(Message((from_id, "DS"), text, author_id, photos=photo))
