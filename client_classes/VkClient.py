        ...
        for event in longpoll.listen():
            if event.type == VkBotEventType.MESSAGE_NEW:
                ...
                if event.from_chat:
                    ...
                    self.compute_massage(Message((from_id, "VK"), text, author_id, chat_name=chat_name, is_owner=is_owner, photos=photo))
                else:
                    from_id = event.object.message["from_id"]
                    self.compute_massage(Message((from_id, "VK"), text, author_id, photos=photo))
